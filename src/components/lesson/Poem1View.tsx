/**
 * Poem 1 — "Daffodils" (William Wordsworth). Uses the shared ExerciseKit
...
import {
  ...
  POEM1_GLOSSARY,
  POEM1_PRE_READING,
  POEM1_PRE_READING_STRUCTURES,
  POEM1_PRE_READING_TRANSLATIONS,
  POEM1_QUESTIONS,
  POEM1_QUESTION_STRUCTURES,
  POEM1_QUESTION_TRANSLATIONS,
  POEM1_STANZAS,
  POEM1_STANZA_TRANSLATIONS,
  POEM1_TITLE,
} from "@/data/poem1";
...
export function Poem1View() {
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <section className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center gap-2 text-xs font-semibold text-primary">
          <span className="rounded-full bg-primary/10 px-2.5 py-1">Poem 1 · Reading</span>
        </div>
        <h2 className="mt-2 text-2xl font-bold leading-tight">{POEM1_TITLE}</h2>
        <p className="mt-1 text-sm text-muted-foreground">{POEM1_AUTHOR}</p>

        <div className="mt-4 space-y-5">
          {POEM1_STANZAS.map((stanza, i) => (
            <div key={i} className="rounded-xl border border-border bg-background p-3">
              ...
              <ToggleReveal label="Translate Stanza" hiddenLabel="Hide Burmese" icon={Languages}>
                {(POEM1_STANZA_TRANSLATIONS[i] ?? []).map((line, k) => (
              ...
              <li key={q.id} className="rounded-xl border border-border bg-background p-3">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <ToggleReveal label="Translate Question" icon={Languages}>
                    {POEM1_PRE_READING_TRANSLATIONS[q.id] ?? "မြန်မာ ဘာသာပြန် မရရှိနိုင်ပါ။"}
                  </ToggleReveal>
                  <ToggleReveal label="Question Structure" icon={Sparkles} tone="primary">
                    <StructureBlock formula={POEM1_PRE_READING_STRUCTURES[q.id] ?? ""} />
                  </ToggleReveal>
                </div>
          ...
          <ol className="mt-4 space-y-4">
            {POEM1_QUESTIONS.map((q) => (
              <li key={q.id} className="rounded-xl border border-border bg-background p-3">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  <ToggleReveal label="Translate Question" icon={Languages}>
                    {POEM1_QUESTION_TRANSLATIONS[q.id] ?? "မြန်မာ ဘာသာပြန် မရရှိနိုင်ပါ။"}
                  </ToggleReveal>
                  <ToggleReveal label="Question Structure" icon={Sparkles} tone="primary">
                    <StructureBlock formula={POEM1_QUESTION_STRUCTURES[q.id] ?? ""} />
                  </ToggleReveal>
                </div>
